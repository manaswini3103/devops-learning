...
- while creating organization it might throw an error like below, as I am working in my comany laptop and could be because we selected the free tier option.  
![DevOps Tools Pipeline](../images/organization.jpg)
- Then it will ask to move to pay as you go as shown below, and we need to move to pay as you go otherwise you will not be able to create the organisation.  
 - You only pay if:
...
- We can even create budgets to not charged for using their serves by setting the budget and it's percentage.  
...
- After createing project, we need to download maven project from any online repo (https://github.com/rahulshettyacademy/MavenHelloWorld) or create it and build it to get the war file.
  - git clone https://github.com/rahulshettyacademy/MavenHelloWorld in VS code
  - Go to github, create a new repostory with the same as online repo, come to VS code go to that repo.
...
- Then we need to start the application server, got to bin folder and double click on **startup** file. Then a terminal will open that server will be starting if we get any error stating port 8080 is already blocked give the below commands taken from stack overflow:  
...
- So when we change something in the code and commit it, automatically the build should be triggered means it should test should run making sure nothing is breaking with the new changes.
...
- Go to the Azure Devops Organization, and your project click, go to pipelines and create a new pipelines,  select GitHub -> repository -> configure your pipeline (select maven) -> output: a YAML file is created (we can change the **pool name** and **JDK version** if it not matching according to our requirements) -> save it, commit to main branch -> then then the created YAML file will be commited to our Git repo.
- Then run the pipeline by selecting our branch, we could an error like below, then we need to fill a form by going to https://aka.ms/azpipelines-parallelism-request or wan create our own agent.  
![DevOps Tools Pipeline](../images/error.jpg)

## Self Hosted Agent

- We can wait after filling the form or we can create a **Self Hosted Agent** (machine you control that runs your Azure DevOps pipeline jobs instead of Microsoft’s hosted machines.)
...
Then copy the token and give it it was asked in Terminal
...
- If it's showing Listening for Jobs we need to go do out things
- Then open the GitHub and change the YAML file to  
```YAML
trigger:
- main
...
- If we want to put our code in Azure repos give, go to azure repos and we choose 2nd option
  ...
  - git remote -v  
    ...
    origin  https://github.com/manaswini3103/MavenHelloWorld (fetch)  
    origin  https://github.com/manaswini3103/MavenHelloWorld (push)  
- Now we are creating a new pipeline linked to Azure Repo Git, select the Name and branch save and run the build. And if you want to run the build automatically, like when we changed some thing in our local machine we need to add, commit and push it to azure repo like **git push azure main**, then the build will automatically run. If we change something directly in Azure Repo we can het it back to local using **git pull azure main**
